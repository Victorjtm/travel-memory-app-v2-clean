...
export class ActividadesItinerariosComponent implements OnInit {
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ PROPIEDADES PARA MODALES
  mostrarModalGPX = false;
  mostrarModalMapa = false;
  mostrarModalEstadisticas = false;
  mostrarModalGPXMapa = false;
  mostrarReproductorAnimado = false; // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVA PROPIEDAD
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ ESTADOS PARA CONFIGURACION DE VIDEO GPX
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ PROPIEDADES PARA MAPA GPX
  mapaGPX: any = null;
  coordenadasGPX: any[] = [];

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVAS PROPIEDADES: Panel de estadÃƒÆ’Ã‚Â­sticas
  showSidePanel = true;
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVAS PROPIEDAD: EstadÃƒÆ’Ã‚Â­sticas completas para el panel
  estadisticasGPX: {
    distanciaKm?: string;
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVAS PROPIEDADES PARA ANIMACIÃƒÆ’Ã¢â‚¬Å“N
  gpxTextAnimacion: string = '';
  multimediaAnimacion: any[] = [];
  desgloseTransporteAnimacion: any[] = [];
  actividadAnimacion: any = null; // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVA PROPIEDAD
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ MODO ALTA FIDELIDAD (visual_session.json)
  visualSessionData: any = null;
  isHighFidelityMode = false;
  private visualSessionGroup: any = null;

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ COLORES POR MODO DE TRANSPORTE (Fallback GPX)
  ...
  // Paleta de colores directa por nombre de modo (idÃƒÆ’Ã‚Â©ntica a GpxAnimationComponent)
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ CACHÃƒÆ’Ã¢â‚¬Â° DE DIRECCIONES PARA LEAFLET (Fase 4 - Bloque C)
    ...
    this.actividadService.getByViajeYItinerario(this.viajePrevistoId, this.itinerarioId)
      .subscribe({
        next: actividades => {
          ...
          console.log('NÃƒÆ’Ã‚Âºmero de actividades:', this.actividades.length);
        },
  ...
  // ==========================================
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ CONFIGURACIÃƒÆ’Ã¢â‚¬Å“N Y GENERACIÃƒÆ’Ã¢â‚¬Å“N DE VÃƒÆ’Ã‚ÂDEO GPX
  // ==========================================
  ...
  async generarVideo(): Promise<void> {
    ...
    try {
      ...
      this.progresoVideo = {
        fase: 'cargando',
        porcentaje: 0,
        mensaje: 'Preparando datos para el vÃƒÆ’Ã‚Â­deo...'
      };

      // 1. Obtener GPX y parsear (usando pipeline canÃƒÆ’Ã‚Â³nico con flatten)
      this.progresoVideo.mensaje = 'Resolviendo ruta GPX canÃƒÆ’Ã‚Â³nica...';
      ...
      this.progresoVideo.mensaje = 'Sincronizando fotos, vÃƒÆ’Ã‚Â­deos y audios...';
      ...
      console.error('Error generando vÃƒÆ’Ã‚Â­deo:', error);
      alert('Error al generar el vÃƒÆ’Ã‚Â­deo: ' + (error instanceof Error ? error.message : 'Error desconocido'));
    } finally {
  ...
  actualizarActividad(actividad: Actividad): void {
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Navegando al formulario de ediciÃƒÆ’Ã‚Â³n para actividad:', actividad.id);

    // Navegar al formulario de ediciÃƒÆ’Ã‚Â³n con todos los parÃƒÆ’Ã‚Â¡metros necesarios
    this.router.navigate([
      ...
      if (success) {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NavegaciÃƒÆ’Ã‚Â³n exitosa');
      } else {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error en la navegaciÃƒÆ’Ã‚Â³n');
      }
    }).catch(err => {
      console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error navegando:', err);
    });
  }
  ...
  verArchivos(actividadId: number, event: Event): void {
    ...
    this.router.navigate(url).catch(err => {
      console.error('Error en navegaciÃƒÆ’Ã‚Â³n:', err);
    });
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ MEJORADO: Ver GPX con Alta Fidelidad si existe visual_session.json
  verGPX(actividadId: number): void {
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â Iniciando Ver GPX para actividad:', actividadId);
    ...
    // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ PASO 1: Cargar estadÃƒÆ’Ã‚Â­sticas (paralelo con los demÃƒÆ’Ã‚Â¡s pasos)
    this.actividadService.obtenerEstadisticas(actividadId).subscribe({
      next: (stats) => {
        ...
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ EstadÃƒÆ’Ã‚Â­sticas mapeadas:', this.estadisticasGPX);
      },
      error: err => console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Error cargando estadÃƒÆ’Ã‚Â­sticas:', err)
    });

    // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ PASO 2: Intentar cargar visual_session.json (Alta Fidelidad)
    //   ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ Independientemente del resultado, despuÃƒÆ’Ã‚Â©s cargamos el GPX como base de coordenadas.
    //   NOTA: las capas estÃƒÆ’Ã‚Â¡n en la raÃƒÆ’Ã‚Â­z del JSON (sessionData.layers), NO en sessionData.mapState.layers
    this.actividadService.obtenerVisualSession(actividadId).subscribe({
      next: (sessionData) => {
        // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ FIX: leer layers desde la raÃƒÆ’Ã‚Â­z del JSON, no desde mapState
        ...
        if (layers && layers.length > 0) {
          ...
          console.log(`ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¨ [Alta Fidelidad] visual_session.json cargado. Capas: ${layers.length}`);
        } else {
          console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â [Alta Fidelidad] JSON sin capas vÃƒÆ’Ã‚Â¡lidas. Activando modo Legacy.');
        }
        this.cargarGPXYAbrirModal(actividadId);
      },
      error: (err) => {
        // 404 es esperado si la actividad no tiene sesiÃƒÆ’Ã‚Â³n visual ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ fallback limpio
        const statusCode = err?.status;
        if (statusCode === 404) {
          console.log('ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â [Legacy] No hay visual_session.json para esta actividad. Usando GPX.');
        } else {
          console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â [Legacy] Error descargando visual_session.json:', err?.message);
        }
        this.isHighFidelityMode = false;
        this.cargarGPXYAbrirModal(actividadId);
      }
  ...
  private cargarGPXYAbrirModal(actividadId: number): void {
    this.trackEditorService.resolveCanonicalGpxXml(actividadId, { flattenSegments: true }).subscribe({
      ...
      error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error obteniendo GPX:', err)
    });
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Animar GPX con multimedia (MEJORADO: Carga estadÃƒÆ’Ã‚Â­sticas primero)
  animarGPX(actividadId: number): void {
    console.log('ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¬ Iniciando proceso de animaciÃƒÆ’Ã‚Â³n para actividad:', actividadId);

    // 1. PASO 1: Asegurar que tengamos las estadÃƒÆ’Ã‚Â­sticas para los segmentos de transporte
    this.actividadService.obtenerEstadisticas(actividadId).subscribe({
      next: (stats) => {
        console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  EstadÃƒÆ’Ã‚Â­sticas cargadas para animaciÃƒÆ’Ã‚Â³n:', stats);
        this.estadisticasGPX = {
          distanciaKm: stats.distancia?.km || '0.00',
          distanciaMetros: stats.distancia?.metros || 0,
          duracion: stats.duracion || { formateada: '00:00:00', segundos: 0 },
        ...
        // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Intentar cargar visual_session.json antes de lanzar la animaciÃƒÆ’Ã‚Â³n
        this.actividadService.obtenerVisualSession(actividadId).subscribe({
          next: (sessionData) => {
            const layers = sessionData?.layers || sessionData?.mapState?.layers;
            if (layers && layers.length > 0) {
              ...
              this.visualSessionData = sessionData;
              this.isHighFidelityMode = true;
              console.log('ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¨ [AnimaciÃƒÆ’Ã‚Â³n] Modo Alta Fidelidad activado.');
            }
      ...
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando estadÃƒÆ’Ã‚Â­sticas para animaciÃƒÆ’Ã‚Â³n:', err);
  ...
  /** Paso 2 y 3 de la animaciÃƒÆ’Ã‚Â³n: Multimedia y GPX */
      ...
      next: (archivos: any[]) => {
        ...
        this.trackEditorService.resolveCanonicalGpxXml(actividadId, { flattenSegments: true }).subscribe({
          next: (gpxText) => {
            ...
            console.log('ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¬ Lanzando reproductor animado con', this.desgloseTransporteAnimacion.length, 'segmentos');
            this.mostrarReproductorAnimado = true;
            this.cdr.detectChanges();
          },
          error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error obteniendo GPX para animaciÃƒÆ’Ã‚Â³n:', err)
        });
      },
      error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error obteniendo multimedia para animaciÃƒÆ’Ã‚Â³n:', err)
    });
  }
  ...
  parseGPX(gpxText: string): void {
    try {
      ...
      console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ GPX parseado. Coordenadas vÃƒÆ’Ã‚Â¡lidas extraÃƒÆ’Ã‚Â­das:', this.coordenadasGPX.length);
      ...
      for (let i = 0; i < wpts.length; i++) {
        ...
        if (name === 'Punto de Giro' || name === 'Punto de Guardado' || name === 'Turn Around') {
          ...
          console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Punto de giro detectado en GPX:', this.turningPoint);
          break;
        }
      }
    } catch (e) {
      console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ ExcepciÃƒÆ’Ã‚Â³n atrapada en parseGPX:', e);
      this.coordenadasGPX = [];
    }
  }

  // Inicializar mapa Leaflet con satÃƒÂ©lite y fotos
  inicializarMapaGPX(): void {
    if (this.coordenadasGPX.length === 0) {
      console.warn('Ã¢Å¡Â Ã¯Â¸Â No hay coordenadas para mostrar');
      return;
    }
    ...
    import('leaflet').then(L => {
      ...
      if (!container) {
        console.error('Ã¢ÂÅ’ Contenedor del mapa no encontrado');
        return;
      }
      container.innerHTML = '';
      ...
      try {
        ...
        const layersControl = L.control.layers(
          { 'SatÃ©lite': satellite, 'Mapa': streets },
          {},
          { position: 'topleft' }
        ).addTo(this.mapaGPX);
        ...
        L.marker(this.coordenadasGPX[0], { icon: inicioIcon }).bindPopup('Ã°Å¸ÂÂ Inicio').addTo(this.mapaGPX);
        ...
        L.marker(this.coordenadasGPX[this.coordenadasGPX.length - 1], { icon: finIcon }).bindPopup('Ã°Å¸ÂÂ Fin').addTo(this.mapaGPX);

        // Marcadores de fotos/vÃƒÂ­deos (Popups enriquecidos desde Phase 2.6)
        ...
        console.error('Ã¢ÂÅ’ Error inicializando Leaflet:', error);
      }
    });
  }
    ...
    archivos.forEach((item, index) => {
      ...
      if (archivo.geolocalizacion) {
        if (this.direccionesCache[archivo.geolocalizacion]) {
          ...
          locationTag = `<div id="lugar-popup-${archivo.id}" style="font-size: 11px; color: #64748b; margin-top: 4px;"><i class="fa fa-map-marker"></i> <span class="lugar-texto">ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â Cargando ubicaciÃƒÆ’Ã‚Â³n...</span></div>`;
        }
      }
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: Cargar fotos y videos desde backend y aÃƒÆ’Ã‚Â±adirlas al mapa con agrupaciÃƒÆ’Ã‚Â³n
  private cargarYAnadirFotos(): void {
    if (!this.mapaGPX || !this.actividadSeleccionada) {
      console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Mapa o ID de actividad no disponible');
      return;
    }
    ...
    this.http.get<any[]>(urlArchivos).subscribe({
      next: (archivos: any[]) => {
        ...
        this.http.get<any[]>(urlAsociados).subscribe({
          ...
          error: (err) => {
            console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Error al cargar archivos asociados, continuando sin audios', err);
            this.procesarArchivosYMarcadores(archivos, []);
          }
        });
      },
      error: err => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando archivos:', err);
      }
    });
  }
  ...
  private procesarArchivosYMarcadores(archivos: any[], asociados: any[]): void {
    ...
    multimedia.forEach(archivo => {
      ...
      // 2. Extraer Lugar (Parseo conservador de la ubicaciÃƒÆ’Ã‚Â³n texto)
      try {
    ...
    const archivosConCoordenadas = multimedia.map((archivo: any) => {
      try {
        ...
        console.warn(`ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Error parseando ${archivo.nombreArchivo}:`, err);
      }
      return null;
    }).filter(Boolean);
    ...
    // ÃƒÂ¢Ã…â€œÃ‚Â¨ FASE 2: Sincronizar array lineal con los grupos del mapa
    this.fotosActividad = [];
    ...
    grupos.forEach((grupo, index) => {
      ...
      // Alimentar la galerÃƒÆ’Ã‚Â­a lateral con el orden y nÃƒÆ’Ã‚Âºmero exacto del mapa
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Agrupar archivos por coordenadas cercanas
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: AÃƒÆ’Ã‚Â±adir marcador de grupo con contador y nÃƒÆ’Ã‚Âºmero secuencial
    ...
    import('leaflet').then(L => {
      ...
      const grupoIcon = L.divIcon({
        ...
        <div style="display: flex; flex-direction: column; align-items: center;">
          <!-- Pin clÃƒÆ’Ã‚Â¡sico mÃƒÆ’Ã‚Â³vil -->
          ...
          <!-- Badge numÃƒÆ’Ã‚Â©rico mÃƒÆ’Ã‚Â³vil -->
      ...
      // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Guardar referencia del marcador en cada archivo para sincronizaciÃƒÆ’Ã‚Â³n
      archivos.forEach(a => {
      ...
      // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: InteracciÃƒÆ’Ã‚Â³n de Dos Pasos (Bloque D)
      // Eliminamos el evento click sobre el marcador que forzaba la apertura directa del visor.
      // Ahora Leaflet abrirÃƒÆ’Ã‚Â¡ naturalmente el popup enriquecido.
      ...
      marker.on('popupopen', (e: any) => {
        ...
        // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: Disparar la geocodificaciÃƒÆ’Ã‚Â³n inversa de Nominatim bajo demanda
        archivos.forEach((item: any) => {
          ...
          if (arch.geolocalizacion && !this.direccionesCache[arch.geolocalizacion]) {
            ...
            }).catch(err => console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Error obteniendo direccion para popup', err));
          }
        });
      });

      console.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Marcador #${numeroSecuencial} aÃƒÆ’Ã‚Â±adido: ${cantidadArchivos} archivo(s) en [${lat}, ${lng}]`);
    });
  }

  // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: Abrir modal con foto o video
  private abrirModalMultimedia(rutaArchivo: string, nombre: string, tipo: string): void {
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Abriendo archivo:', nombre, 'Tipo:', tipo);
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â rutaArchivo:', rutaArchivo);

    const backendUrl = environment.apiUrl;
    const urlArchivo = `${backendUrl}/uploads/${rutaArchivo}`;

    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â€œÃ‚Â¼ÃƒÂ¯Ã‚Â¸Ã‚Â URL final del archivo:', urlArchivo);
  ...
  `;
    ...
    const emoji = esFoto ? 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â·' : 'ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¬';
    ...
    // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ CREAR ELEMENTO SEGÃƒÆ’Ã…Â¡N EL TIPO
    let mediaElement: HTMLImageElement | HTMLVideoElement;

    if (esFoto) {
      // ÃƒÂ¢Ã…â€œÃ‚Â¨ MEJORADO: Usar el visualizador avanzado en nueva ventana
      const backendUrl = environment.apiUrl;
      const urlArchivo = `${backendUrl}/uploads/${rutaArchivo}`;
      ...
      // CÃƒÆ’Ã‚Â³digo antiguo para fallback o por si falla la ventana
      ...
      imagen.onerror = () => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando imagen desde:', urlArchivo);
      ...
      video.onerror = () => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando video desde:', urlArchivo);
    ...
    // ÃƒÂ¢Ã…â€œÃ‚Â¨ CONTENEDOR DE BOTONES
  ...
  `;
    ...
    // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO BOTÃƒÆ’Ã¢â‚¬Å“N: VER EN LISTA DE ARCHIVOS
    const btnVerArchivo = document.createElement('button');
    btnVerArchivo.textContent = 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â Ver en Archivos';
  ...
  `;
    ...
    btnVerArchivo.addEventListener('click', () => {
      console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â€ Navegando a archivos...');
      modal.remove();

      // Navegar a la pÃƒÆ’Ã‚Â¡gina de archivos
      this.router.navigate([
        '/viajes-previstos',
        this.viajePrevistoId,
        'itinerarios',
    ...
    console.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Modal de ${etiqueta} abierto`);
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Abrir modal con grupo de archivos (carrusel)
  private abrirModalGrupo(archivos: any[], numeroSecuencial: number): void {
    console.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Abriendo grupo #${numeroSecuencial} con ${archivos.length} archivo(s)`);
    ...
    // FunciÃƒÆ’Ã‚Â³n para actualizar el contenido del modal
    const actualizarContenido = () => {
      ...
      // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: Click en la imagen para abrir visualizador avanzado
      if (esFoto) {
    ...
    console.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Modal de grupo abierto`);
  }

  // Ver Mapa PNG - Muestra en modal
  verMapa(actividadId: number): void {
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â€Ã‚ÂºÃƒÂ¯Ã‚Â¸Ã‚Â Obteniendo mapa para actividad:', actividadId);
    this.actividadSeleccionada = actividadId;
    this.actividadService.obtenerMapa(actividadId).subscribe({
      next: (blob) => {
        ...
        reader.onload = (e: any) => {
          ...
          console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Mapa cargado en modal');
        };
        reader.readAsDataURL(blob);
      },
      error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error obteniendo mapa:', err)
    });
  }

  // Ver EstadÃƒÆ’Ã‚Â­sticas - Muestra en modal
  verEstadisticas(actividadId: number): void {
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  Obteniendo estadÃƒÆ’Ã‚Â­sticas para actividad:', actividadId);
    this.actividadSeleccionada = actividadId;
    this.actividadService.obtenerEstadisticas(actividadId).subscribe({
      next: (datos) => {
        this.estadisticasActuales = datos;
        this.mostrarModalEstadisticas = true;
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ EstadÃƒÆ’Ã‚Â­sticas cargadas:', datos);
      },
      error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error obteniendo estadÃƒÆ’Ã‚Â­sticas:', err)
    });
  }
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Toggle panel de estadÃƒÆ’Ã‚Â­sticas
  toggleSidePanel(): void {
    this.showSidePanel = !this.showSidePanel;
    console.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬â€Ã‚ÂºÃƒÂ¯Ã‚Â¸Ã‚Â Panel lateral: ${this.showSidePanel ? 'VISIBLE' : 'OCULTO'}`);
    
    // Invalida el tamaÃƒÆ’Ã‚Â±o del mapa poco despuÃƒÆ’Ã‚Â©s para que se adapte al contenedor redimensionado
    if (this.mapaGPX) {
      setTimeout(() => {
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Expandir/contraer panel
  togglePanelExpanded(): void {
    this.panelExpanded = !this.panelExpanded;
    console.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  Panel: ${this.panelExpanded ? 'EXPANDIDO' : 'CONTRAÃƒÆ’Ã‚ÂDO'}`);
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Ajustar vista del mapa al track completo
  fitMapToTrack(): void {
    if (!this.mapaGPX || this.coordenadasGPX.length === 0) {
      console.warn('ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â No hay mapa o coordenadas disponibles');
      return;
    }
    ...
    import('leaflet').then(L => {
      ...
      console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Vista ajustada al track completo');
    });
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Obtener emoji de transporte
  getTransportEmoji(iconName: string): string {
    const emojiMap: { [key: string]: string } = {
      'walk': 'ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â¶',
      'fitness': 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ†â€™',
      'bicycle': 'ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â´',
      'car': 'ÃƒÂ°Ã…Â¸Ã…Â¡Ã¢â‚¬â€'
    };
    return emojiMap[iconName] || 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â';
  }

  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Formatear distancia
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Formatear duraciÃƒÆ’Ã‚Â³n
  ...
  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ MEJOR OPCIÃƒÆ’Ã¢â‚¬Å“N: Flechas SVG (escalables sin pixelar)
  private addDirectionArrows(L: any, coordinates: any[], color: string = '#FF0000', opacity: number = 1): void {
    ...
    // ÃƒÂ¢Ã…â€œÃ‚Â¨ FASE 1 (IteraciÃƒÆ’Ã‚Â³n 3): Chevron minimalista, muy sutil
    ...
    for (let i = interval; i < coordinates.length; i += interval) {
      ...
      const arrowIcon = L.divIcon({
        ...
        <svg width="16" height="16" viewBox="0 0 32 32" 
          ...
          <!-- LÃƒÆ’Ã‚Â­nea de color semÃƒÆ’Ã‚Â¡ntico -->
          <path d="M 6 24 L 16 8 L 26 24" 
                fill="none" 
                stroke="${color}" 
                stroke-width="3"
    ...
    console.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ ${Math.floor(totalPoints / interval)} flechas SVG aÃƒÆ’Ã‚Â±adidas`);
  }


  // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ NUEVO: Calcular ÃƒÆ’Ã‚Â¡ngulo entre dos puntos
  ...
  // Ã¢Å“â€¦ NUEVO: Funciones trackBy para rendimiento
  ...
  // Ã¢Å“â€¦ NUEVO: FunciÃƒÂ³n para determinar si el archivo es un vÃƒÂ­deo
  ...
  centrarEnFoto(foto: any): void {
    if (foto.marcadorRef && this.mapaGPX) {
      if (this.coordenadasGPX && this.coordenadasGPX.length > 0) {
        import('leaflet').then(L => {
          // Aseguramos que la ruta completa estÃ© visible y centrada
          const bounds = L.latLngBounds(this.coordenadasGPX);
          this.mapaGPX.fitBounds(bounds, { padding: [50, 50] });
  ...
  // ÃƒÂ¢Ã…â€œÃ‚Â¨ NUEVO: Convertir coordenadas a direcciÃƒÆ’Ã‚Â³n legible bajo demanda usando Nominatim (Fase 4 - Bloque C)
  private async obtenerDireccion(geolocalizacion: string): Promise<string> {
    if (!geolocalizacion || geolocalizacion === 'No disponible') {
      return 'UbicaciÃƒÆ’Ã‚Â³n no disponible';
    }
    ...
    try {
      ...
      if (geolocalizacion.includes('{')) {
        ...
        return 'Formato invÃƒÆ’Ã‚Â¡lido';
      }

      if (isNaN(lat) || isNaN(lon)) return 'Coordenadas invÃƒÆ’Ã‚Â¡lidas';
      ...
      try {
        ...
        clearTimeout(timeoutId);
        const fallback = `${lat.toFixed(5)}Ãƒâ€šÃ‚Â°, ${lon.toFixed(5)}Ãƒâ€šÃ‚Â°`;
      ...
      if (!direccion) direccion = `${lat.toFixed(5)}Ãƒâ€šÃ‚Â°, ${lon.toFixed(5)}Ãƒâ€šÃ‚Â°`;

      this.direccionesCache[geolocalizacion] = direccion;
      return direccion;
    } catch (e: any) {
      console.error('Error obteniendo direcciÃƒÆ’Ã‚Â³n:', e);
      try {
        const coords = JSON.parse(geolocalizacion);
        const fallback = `${coords.latitud.toFixed(5)}Ãƒâ€šÃ‚Â°, ${coords.longitud.toFixed(5)}Ãƒâ€šÃ‚Â°`;
        this.direccionesCache[geolocalizacion] = fallback;
        return fallback;
      } catch {
        return 'UbicaciÃƒÆ’Ã‚Â³n no disponible';
  ...
  // ÃƒÂ¢Ã…â€œÃ‚ÂÃƒÂ¯Ã‚Â¸Ã‚Â EDITOR DE RECORRIDO GPX (Fase 1 y 2.1)
  // ÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚ÂÃƒÂ¢Ã¢â‚¬ÂÃ‚Â

  abrirEditorTrack(actividadId: number): void {
    console.log('ÃƒÂ¢Ã…â€œÃ‚ÂÃƒÂ¯Ã‚Â¸Ã‚Â Abriendo editor de recorrido para actividad:', actividadId);
        ...
        this.trackEditorService.getSegments(actividadId).subscribe({
          next: (segments) => {
            if (segments && segments.length > 0) {
              // Ya existe migraciÃƒÆ’Ã‚Â³n, construir track componiendo segmentos con Event Sourcing
              ...
              this.actividadService.obtenerGPX(actividadId).subscribe({
                next: (blob) => {
                  ...
                  reader.onload = (e: any) => {
                    ...
                    this.trackEditorService.createSegment(actividadId, this.gpxPointsEditor, 'original').subscribe({
                      next: () => console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ GPX base migrado exitosamente a segments (source: original)'),
                      error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error migrando GPX base a segments:', err)
                    });
                ...
                error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando GPX para editor:', err)
              });
            }
          },
          error: err => console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando segments para editor:', err)
        });
      },
      error: err => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error cargando TrackEdits para editor:', err);
        this.editsEditor = [];
      }
    });
  }
  ...
  onTrackEditRequest(event: any): void {
    ...
    console.log('ÃƒÂ¢Ã…â€œÃ‚ÂÃƒÂ¯Ã‚Â¸Ã‚Â Guardando ediciÃƒÆ’Ã‚Â³n:', event);
    ...
    this.trackEditorService.createTrackEdit(edit).subscribe({
      next: (savedEdit) => {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ EdiciÃƒÆ’Ã‚Â³n guardada con ID:', savedEdit.id);
        this.editsEditor = [...this.editsEditor, savedEdit];
        this.cdr.detectChanges();
      },
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando ediciÃƒÆ’Ã‚Â³n:', err);
        alert('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando la ediciÃƒÆ’Ã‚Â³n. Revisa la consola.');
      }
    });
  }
  ...
  onAppendRequest(event: { points: { lat: number; lng: number }[] }): void {
    ...
    console.log('ÃƒÂ¢Ã…Â¾Ã¢â‚¬Â¢ Guardando Append:', event.points.length, 'puntos');

    this.trackEditorService.createSegment(this.actividadEditorId, event.points, 'user-append').subscribe({
      next: (resp) => {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Segment append guardado con id:', resp.id, 'order:', resp.segmentOrder);

        // Calcular timestamps monotÃƒÆ’Ã‚Â³nicos y concatenar al array local
      ...
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando segment append:', err);
        alert('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando el tramo. Revisa la consola.');
      }
    });
  }
  ...
  onInsertRequest(event: { points: { lat: number; lng: number; time?: string }[] }): void {
    ...
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â Guardando Insert:', event.points.length, 'puntos');
    ...
    this.trackEditorService.createSegment(this.actividadEditorId, payload, 'user-insert').subscribe({
      next: (resp) => {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Segment insert guardado con id:', resp.id, 'order:', resp.segmentOrder);
      ...
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando segment insert:', err);
        alert('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando el tramo insertado. Revisa la consola.');
      }
    });
  }
  ...
  onOverrideModeRequest(event: { points: { lat: number; lng: number; time?: string; mode?: string }[] }): void {
    ...
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Guardando Override de Modo de Transporte:', event.points.length, 'puntos');
    ...
    this.trackEditorService.createSegment(this.actividadEditorId, payload, 'user-override').subscribe({
      next: (resp) => {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Segment override guardado con id:', resp.id, 'order:', resp.segmentOrder);
      ...
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando segment override:', err);
        alert('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando el cambio de transporte. Revisa la consola.');
      }
    });
  }
  ...
  onDeleteRequest(event: { anchorA: any; anchorB: any }): void {
    ...
    console.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Guardando Delete de Tramo:', event.anchorA, event.anchorB);

    // Mapear al formato de persistencia plano: sÃƒÆ’Ã‚Â³lo guardamos las dos anclas
    const payload = [
    ...
    this.trackEditorService.createSegment(this.actividadEditorId, payload, 'user-delete').subscribe({
      next: (resp) => {
        console.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Segment delete guardado con id:', resp.id, 'order:', resp.segmentOrder);
      ...
      error: (err) => {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando segment delete:', err);
        alert('ÃƒÂ¢Ã‚ÂÃ…â€™ Error guardando el borrado del tramo. Revisa la consola.');
      }
    });
  }
